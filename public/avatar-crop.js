  ...
  function createCropModal({ imageSrc, onConfirm, onCancel }) {
    ...
    overlay.innerHTML = `
      <div class="crop-modal__card panel" role="dialog" aria-modal="true">
        ...
        <p class="panel__subtitle">Выберите видимую область</p>
    ...
    function layout() {
      ...
      const vh = Math.min(360, viewport.clientWidth);
      ...
      frameSize = Math.min(canvas.width, canvas.height, 220) * 0.85;
    ...
    overlay._cleanup = () => window.removeEventListener("resize", layout);
    overlay.addEventListener("remove", () => overlay._cleanup?.());
  }
